...
double distance;
double vectemp[8];
SDoublePlane tempo(input.rows(),input.cols());
for(int i=8;i<input.rows()-8;i++)
{
for(int j=8;j<input.cols()-8;j++)
{
if(input[i][j]==0)
//if(input[i][j]!=0)
{
for(int e=i-8;e<i+8;e+=4)
{
for(int r=j-8;j<j+8;j+=4)
{
for(int k=0;k<8;k++)
{
vectemp[k]=0;
...
if(a-1<0 && b-1 >=0) 
{
tempo[a][b]=sqrt       (         pow(input[a+1][b],2)+pow((input[a][b+1]-input[a][b-1]),2)                 );
theta = atan ((input[a][b+1]-input[a][b-1])/(input[a+1][b]));

}
if(a-1>=0 && b-1 <0)
{
tempo[i][j]=sqrt       (         pow(input[a][b+1],2)+pow((input[a+1][b]-input[a-1][b]),2)                 );
theta = atan ((input[a][b+1])/(input[a+1][b]-input[a-1][b]));
}
if(a-1<0 && b-1 <0)
{
tempo[i][j]=sqrt       (         pow(input[a][b+1],2)      +           pow(input[a+1][b],2)                 );
theta = atan ((input[a][b+1])/(input[a+1][b]));
}
else
{
tempo[i][j]=sqrt       (         pow((input[a][b+1]-input[a][b-1]),2)+pow((input[a+1][b]-input[a-1][b]),2)                 );
theta = atan ((input[a][b+1]-input[a][b-1])/(input[a+1][b]-input[a-1][b]));
}
if(theta <0)
{
theta = floor(theta + 360);
}
distance = sqrt       (         pow((j-b),2)+pow((i-a),2)                 );
tempo[a][b]=tempo[a][b]/distance;
rem = floor(theta/45);
vectemp[rem] += tempo[i][j];
}
}
for(int d=0;d<=rem;d++)
{
if(count<=128)
{
descriptor.vector[count++] = vectemp[d];
}
}
...
for(int j=0;j<input.cols();j++)
{
R=(gaussxsquare[i][j]*gaussysquare[i][j]) - (pow(gaussxy[i][j],2))- ((0.05)*(gaussxsquare[i][j]+gaussysquare[i][j]));
if(R>3000)
{
c.row=i;
c.col=j;
...
//invariant_descriptors(output, points);
 ...
 //SImageIO::write_png_file("harris.png", output , output , output);
//for(int i=4;i<input.rows())
  return points;
...
cout<<"upto here";
output_points = find_corners(gaussed,gradient_x,gradient_y);
//cout<<output_points.size()<<"read this";
//for(int i=0;i<output_points.size();i++)
//{
//cout<<output_points.at(i).col<<endl;
//}
