...
//
//Length of descriptor vector is 128
...
vector<Coordinate> find_corners_tomasi(const SDoublePlane &input,const SDoublePlane &input_x,const SDoublePlane &input_y)
{
	...
	gaussxsquare=gaussian_filter(xsquare,1.5,5);
	gaussysquare=gaussian_filter(ysquare,1.5,5);
	gaussxy=gaussian_filter(xy,1.5,5);
	for(int i=0;i<input.rows();i++)
	{
		for(int j=0;j<input.cols();j++)
		{
...
SImageIO::write_png_file("TomasiCorner.png", output , output , output);
}

vector<Coordinate> find_corners_harris(const SDoublePlane &input,const SDoublePlane &input_x,const SDoublePlane &input_y,double sigma,int size,string filename,int turn)
{
...
double* thresh(double r,double l, double t, double b,double *tempc)
{
    tempc[0]=sqrt(pow((r-l),2)+pow((b-t),2));
	tempc[1] = atan((r-l)/(b-t))* double(180/PI);
	return tempc;
}
...
for(i=100;i<limit1;i++)
{
	for(j=100;j<limit2;j++)
	{
		d=0;
		for(k=0;k<128;k++)
		{
...
//cout<<d<<endl;
		...
		//ims.intensity=image1[image1_coordinates.at(i).row][image1_coordinates.at(i).col]-image1[image1_coordinates.at(i).row][image1_coordinates.at(i).col]
...
void stitch(SDoublePlane &image1,SDoublePlane &image2,SDoublePlane &gradient_x1,SDoublePlane &gradient_y1,SDoublePlane &gradient_x2,SDoublePlane &gradient_y2,string output_filename,string image1_filename,string image2_filename)
{
	...
	int left,right;
int jayesh,jayesh1;	
vector<Coordinate> image1_coordinates;
image1_coordinates = find_corners_harris(image1,gradient_x1,gradient_y1,1,3,image1_filename,0);
	vector<Descriptor> image1_descriptors; 
image1_descriptors=invariant_descriptors(image1, image1_coordinates);
	ofstream ofs(output_filename.c_str());
	vector<Coordinate> image2_coordinates;
image2_coordinates  = find_corners_harris(image2,gradient_x2,gradient_y2,1,3,image2_filename,1);
	vector<Descriptor> image2_descriptors;
image2_descriptors = invariant_descriptors(image2, image2_coordinates);
	int variable=0;
	int counter=0;
	int maximum;
	int count1=0;
...
jayesh = image1_coordinates.at(ptimage1).col;
	 jayesh1 = image2_coordinates.at(ptimage2).col;

	//cout<<xtrans<<" "<<ytrans<<" "<<ptimage1<<" "<<ptimage2<<endl;
cout<<"X translation between two images is  "<<xtrans<<endl;
cout<<"Y translation between two images is  "<<ytrans<<endl;
cout<<"corresponding point in first image is  "<<image1_coordinates.at(ptimage1).row<<"  "<<image1_coordinates.at(ptimage1).col<<endl;
cout<<"corresponding point in second image is  "<<image2_coordinates.at(ptimage2).row<<"  "<<image2_coordinates.at(ptimage2).col<<endl;
	...
	else
	{
		for(int i=image1ylow;i<=image1yhigh;i++)
		{
			for(int j=0;j<=image1xhigh-image1xlow-1;j++)
			{
...
//SDoublePlane input1=gaussian_filter(input,1.5,3);
...
if(image1.rows()>1500 || image1.cols()>1500 || image2.rows()>1500 || image2.cols()>1500)
{
image1=subsample(image1,2);
image2=subsample(image2,2);
}
	//SImageIO::write_png_file("image3.png", image1, image1, image1);
	//SImageIO::write_png_file("image4.png", image2, image2, image2);

SDoublePlane gaussed1 = gaussian_filter(image1,2,3);
	SImageIO::write_png_file("gaussed1.png", gaussed1 , gaussed1 , gaussed1);
	SDoublePlane gaussed2 = gaussian_filter(image2,2,3);
